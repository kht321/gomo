    ...
    for m in pkgutil.walk_packages(root.__path__, root.__name__ + "."):
        ...
        for _, obj in inspect.getmembers(mod, inspect.isclass):
            if getattr(obj, "__name__", "") in {"GomokuGame", "Game"}:
                if hasattr(obj, "make_move") and hasattr(obj, "check_winner"):
                    return obj
...
async def _play_once(a: Agent, b: Agent, board_size: int, max_turns: int, log: Optional[str]):
    ...
    while (not game.check_winner()) and turn < max_turns:
        agent = a if game.current_player.value == "X" else b
        move = await agent.get_move(state)
        row, col = move

        if not game.make_move(row, col):
            print(f"[warning] Invalid move by {agent.agent_id} at ({row},{col}) — skipping turn.")
            break

        state = game.state
        history.append({
            "turn": turn + 1,
            "player": "X" if agent == a else "O",
            "move": (row, col)
        })
        print(_fmt(state))
        print(f"Move: ({row}, {col}) (turn {turn + 1})\n")
        turn += 1
